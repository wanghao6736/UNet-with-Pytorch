"""
Residual UNet implementation using basic modules.
Combines UNet architecture with residual connections.
"""
...
class ResUNet(nn.Module):
    ...
    Uses residual connections in the convolution blocks.
    ...
    def __init__(
        ...
        for feature in features:
            self.downs.append(
                ResidualDoubleConv(
                    in_channels, feature, dropout_p=0.1
                )
            )
            in_channels = feature
        ...
        self.bottleneck = ResidualDoubleConv(
            features[-1], features[-1] * 2, dropout_p=0.2
        )

        # Up path with residual blocks
        for feature in reversed(features):
            ...
            self.ups.append(
                ResidualDoubleConv(
                    feature * 2, feature, dropout_p=0.1
                )
            )
